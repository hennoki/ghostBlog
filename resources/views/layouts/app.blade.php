...
<head>
    ...
    <title>{{ $metaTitle ?: 'Ghost-👁 Blog' }}</title>
...
<body class="bg-gray-50 font-family-karla">
    ...
    <header class="w-full container mx-auto">
        <div class="flex flex-col items-center py-12">
            <a class="font-bold sm:font-thinl text-green-800 uppercase hover:text-green-700 text-5xl"
                href="{{ route('home') }}">
                Ghost-👁 Blog
            </a>
    ...
    <!-- Topic Nav -->
    <nav class="w-full py-4 border-t border-b bg-green-500" x-data="{ open: false }">
        ...
        <div :class="open ? 'block' : 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
            <div
                ...
                <div>
                    <a href="{{ route('home') }}"
                        class="hover:bg-teal-600 hover:text-white rounded py-2 px-4 mx-2">Home</a>
                    @foreach ($categories as $category)
                        <a href="{{ route('by-category', $category) }}"
                            class="hover:bg-teal-600 hover:text-white rounded py-2 px-4 mx-2">{{ $category->title }}</a>
                    @endforeach
                    <a href="{{ route('about-us') }}"
                        class="hover:bg-teal-600 hover:text-white rounded py-2 px-4 mx-2">About
                        us</a>
                </div>
                ...
                <div class="flex items-center">
                    ...
                    @else
                        <a href="{{ route('login') }}"
                            class="hover:bg-green-600 hover:text-white rounded py-2 px-4 mx-2">Login</a>
                        <a href="{{ route('register') }}"
                            class="bg-green-600 text-white rounded py-2 px-4 mx-2">Register</a>
                    @endauth
                </div>
            </div>
        </div>
